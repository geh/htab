...
tableauStart p bi = (configureStats p) >> tableau p 0 bi

tableau :: Params -> Depth -> BranchInfo -> TableauMonad OpenFlag
tableau p depth branchInfo =
                  ...
                  Just (rule,newTodo,newBranch)  -> -- of course then merge newBranch and newTodo
                      do verbose (">> Rule : " ++ show rule)
                         ...
                         case applyRule p rule newBranch newTodo of
                          [newBi] -> tableau p (depth + 1) newBi
                          bis     -> chooseBranch p dsEmpty bis (depth + 1)


chooseBranch :: Params -> DependencySet -> [BranchInfo] -> Depth -> TableauMonad OpenFlag
chooseBranch p currentDepSet (hd:tl) depth =
 do res <- tableau p depth hd
    case res of
     o@(OPEN _)    -> return o
     CLOSED depSet ->
         if backjumping p && not (dsMember depth depSet)
          then return $ CLOSED depSet
          else chooseBranch p (dsUnion currentDepSet depSet) tl depth
